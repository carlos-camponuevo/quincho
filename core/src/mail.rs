...
// The logo (crate::brand, provided at runtime) ships as an inline (cid:)
// attachment — Gmail and Outlook refuse data: URIs, so the image has to travel
// as a real MIME part. Without a logo the report simply has none.
...
pub fn send_html(
    ...
    let mut related = MultiPart::related().multipart(MultiPart::alternative_plain_html(
        plain.to_string(),
        html.to_string(),
    ));
    if let Some(logo) = crate::brand::logo() {
        related = related.singlepart(
            Attachment::new_inline(crate::report::LOGO_CID.to_string())
                .body(logo.to_vec(), ContentType::parse("image/png")?),
        );
    }
