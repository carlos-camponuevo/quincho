...
/// `pins` overrides a service's `image:` with an exact reference
/// (service name -> "repo@sha256:…") — the request flow deploys pinned
/// digests, rollback redeploys previously recorded ones.
pub fn prepare_compose(
    ...
    // fold env_file contents into environment (and apply image pins)
    ...
    for name in names {
        ...
        if let Some(pin) = name.as_str().and_then(|n| pins.get(n)) {
            svc_map.insert(Value::from("image"), Value::from(pin.clone()));
        }
...
/// `pins` — see [`prepare_compose`]; pass an empty map for tag-based deploys.
pub fn run_deploy(
    fs: &MemFs,
    dir: &str,
    target: Target,
    ...
    for (svc, image) in pins {
        eprintln!("   📌 {svc} -> {image}");
    }

    let compose = prepare_compose(fs, dir, &target, pins)?;
...
/// Image reference a running swarm service was created with — the value
/// recorded BEFORE a deploy so a rollback can pin it back. Swarm service
/// names follow `<stack>_<service>`. None: the service does not exist yet
/// (or docker is unreachable) — there is nothing to roll back to.
pub fn service_image(service: &str) -> Option<String> {
...
mod tests {
    ...
    fn merges_env_files_into_environment() {
        ...
        let out = prepare_compose(&fs, "zauat/admin", &Target::WholeStack, &BTreeMap::new()).unwrap();
    ...
    fn filters_services() {
        ...
        let out =
            prepare_compose(&fs, "e/s", &Target::Services(vec!["b".into()]), &BTreeMap::new())
                .unwrap();
        let doc: Value = serde_yaml::from_str(&out).unwrap();
    ...
    fn pins_service_images() {
        ...
        let pins = BTreeMap::from([(
            "a".to_string(),
            "ns/app@sha256:0123456789abcdef".to_string(),
        )]);
        let out = prepare_compose(&fs, "e/s", &Target::WholeStack, &pins).unwrap();
        let doc: Value = serde_yaml::from_str(&out).unwrap();
        assert_eq!(
            doc["services"]["a"]["image"].as_str(),
            Some("ns/app@sha256:0123456789abcdef"),
            "pinned service must carry the exact reference"
        );
        assert_eq!(
            doc["services"]["b"]["image"].as_str(),
            Some("ns/app:latest"),
            "unpinned service keeps its tag"
        );
    }
}
