...
pub const LOGO_CID: &str = "quincho-logo";
...
pub fn document(kind: &str, headline: &str, ok: bool, status: &str, body: String) -> String {
    ...
    let logo = if crate::brand::logo().is_some() {
        format!("<img src=\"cid:{LOGO_CID}\" width=\"96\" alt=\"\" style=\"display:block;border:0;opacity:.85\">")
    } else {
        String::new()
    };
    format!(
